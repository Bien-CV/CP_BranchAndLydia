...
int newTimer(timeLog* log, char* name, int pruneType, int sizeFactor){
	...
	log->records[usableIndex].nbSolutions = 0;
	//log->records[usableIndex].name=calloc(strlen(name)+1,sizeof(char)*2);
	log->records[usableIndex].name=malloc(sizeof(char)*(strlen(name)+1));
...
void stopTimer(timeLog* log,int index,int nbSolutions){
	log->records[index].nbSolutions = nbSolutions;
...
int BranchAndPrune(int taille,int typeOfPruning,int* nbSolutions){
  ...
  pile* dxi;
	*nbSolutions=0;
	...
	pile_Dom** L = malloc(sizeof(pile_Dom*));
	Push_dom(L,*(P->d));
	Length_dom(*L);
  int t=0;
...
void showLogCSV(timeLog* log){
	printf("type de pruning;taille de l'échiquier;nom du timer;durée en temps CPU;Nombre de solutions trouvées\n");
	...
	for (int i=0;i< (log->nbOfRecords );i++){
		...
		printf("%s;%lu;",(log->records)[i].name,(log->records)[i].stop);
		printf("%d\n",(log->records)[i].nbSolutions);
	}
	printf("\n");
}
...
void initLog(timeLog* log){
	...
	log->records=malloc(MAX_NB_OF_TIME_RECORDINGS*sizeof(timeRecord));
	log->nbOfRecords=0;
}

timeLog* newLog(){
	//timeLog* newLog=calloc(1,sizeof(timeLog));
	timeLog* newLog=malloc(sizeof(timeLog));
	initLog(newLog);
	return newLog;
}
...
int main()
{
  ...
  char* buffer=malloc(maxNameLen*sizeof(char));
  int timerIndex;
  int maxNbOfIterations = 11;
  //int* nbSolutions=calloc(1,sizeof(int));
  int* nbSolutions=malloc(sizeof(int));
 /*
  for(int i=4;i<maxNbOfIterations;incrementer(&i)){
	...
	TIMER_ON stopTimer(log, timerIndex,(*nbSolutions));
